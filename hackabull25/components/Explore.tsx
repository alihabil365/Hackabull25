  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchesSearch =
        ...
        item.additionalInfo?.some((info) => info.toLowerCase().includes(search));

      const matchesMin = !filters.minPrice || item.price >= parseFloat(filters.minPrice);
      const matchesMax = !filters.maxPrice || item.price <= parseFloat(filters.maxPrice);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold mb-8 text-black">Marketplace Items</h1>
